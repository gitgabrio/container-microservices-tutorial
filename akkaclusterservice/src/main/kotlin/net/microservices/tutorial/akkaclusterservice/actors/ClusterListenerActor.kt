...
class ClusterListenerActor(private val homeService: HomeService) : AbstractActor() {
    ...
    override fun createReceive(): AbstractActor.Receive {
        return receiveBuilder()
                .match(MemberUp::class.java) { mUp ->
                    ...
                    homeService.nodeStatusMap[mUp.member().uniqueAddress().address()] =
                        HomeService.MemberDetail(mUp.member().roles, HomeService.NODE_STATUS.UP)
                }
                .match(UnreachableMember::class.java) { mUnreachable ->
                    log.info("Member detected as unreachable: {}", mUnreachable.member())
                    homeService.nodeStatusMap[mUnreachable.member().uniqueAddress().address()] =
                        HomeService.MemberDetail(mUnreachable.member().roles, HomeService.NODE_STATUS.UNREACHABLE)
                }
                .match(MemberRemoved::class.java) { mRemoved ->
                    log.info("Member is Removed: {}", mRemoved.member())
                    homeService.nodeStatusMap[mRemoved.member().uniqueAddress().address()] =
                        HomeService.MemberDetail(mRemoved.member().roles, HomeService.NODE_STATUS.REMOVED)
                }
